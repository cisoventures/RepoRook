...
test("stdio server negotiates MCP and exposes all tools", async () => {
  ...
  assert.deepEqual(responses[1].result.tools.map((tool) => tool.name), [
    "scan_repository", "scan_changes", "prioritize_findings", "get_policy_status", "create_findings_baseline", "suppress_finding", "list_findings", "get_finding", "get_remediation_context", "prepare_remediation_plan", "verify_fix", "record_remediation_approval", "export_findings",
  ]);
