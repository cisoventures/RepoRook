...
export function parseGitleaks(raw: unknown, target: string, history = false): Finding[] {
  return array(raw).map((item) => {
    ...
    return {
      ...
      metadata: {
        ...
        ...(history ? { target_kind: "git-history" as const, target: file } : {}),
        ...(history && /^[a-f0-9]{7,64}$/i.test(sourceCommit) ? { source_commit: sourceCommit } : {}),
      },
    } satisfies Finding;
  });
}
...
export class GitleaksScanner implements ScannerAdapter {
  ...
  async run(context: ScannerContext): Promise<ScannerResult> {
    ...
    try {
      const history = context.config.gitHistory;
      let result = history
        ? await runCommand("gitleaks", ["git", context.target, "--report-format", "json", "--report-path", reportPath, "--redact", "--no-banner"], { cwd: context.target })
        : await runCommand("gitleaks", ["dir", context.target, "--report-format", "json", "--report-path", reportPath, "--redact", "--no-banner"], { cwd: context.target });
      if (!history && ![0, 1].includes(result.code) && /unknown command|accepts|usage/i.test(result.stderr)) {
