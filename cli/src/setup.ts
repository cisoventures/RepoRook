...
export function setupInstructions(): string {
  ...
  if (current === "darwin") {
    lines.push("macOS (Homebrew):", "  brew install semgrep gitleaks pip-audit");
  } else if (current === "win32") {
    lines.push("Windows:", "  python -m pip install --user semgrep pip-audit", "  winget install Gitleaks.Gitleaks");
  } else {
    lines.push("Linux:", "  python3 -m pip install --user semgrep pip-audit", "  Install Gitleaks from its signed GitHub release or your distribution package manager.");
  }
