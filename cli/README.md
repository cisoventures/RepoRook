...
Checkov automatically covers detected Terraform, Kubernetes, Helm, Kustomize, Dockerfile, and GitHub Actions files using local built-in policies with uploads and external downloads disabled. A Trivy image scan requires explicit `containerImages` entries plus `--allow-external-targets` on that `scan` or `verify` invocation. RepoRook removes generic `TRIVY_USERNAME` and `TRIVY_PASSWORD` values from the child process; authenticate private registries through host-scoped Docker credentials. Set `gitHistory: true` only when you intend Gitleaks to inspect past commits; normalized evidence remains redacted.
