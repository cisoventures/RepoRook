...
Releases run only from version tags and require approval through the `npm-release` GitHub environment. The package versions in `cli/package.json`, `mcp-server/package.json`, and `service/package.json` must exactly match the tag.
...
3. Create a short-lived granular npm token that can publish the packages and bypass 2FA for automation.
...
6. Confirm all public npm packages and their provenance attestations.
...
1. Update all package versions and the root lockfile, then merge the tested release commit.
2. Tag that exact commit as `v<version>` and push the tag.
3. Review and approve the protected `npm-release` GitHub deployment.
4. Confirm the workflow stages all npm tarballs through OIDC and creates a draft GitHub release with the npm and adapter artifacts.
5. Inspect every entry in npm's **Staged Packages** view, then approve each with 2FA.
6. Verify every version and provenance attestation from the public registry.
