...
| RR-COV-03 | Fixed | Fixture verification, guided-fix, and policy gates are unconditional CI steps; `action/supply-chain-policy.test.mjs` asserts those workflow steps remain outside optional scanner-setup conditions. |
| RR-EVD-01 | Fixed | Findings, standalone scan receipts, priorities, remediation plans, verification receipts, and approvals require repository-path-bound HMAC authentication. Strict consumers verify before use; forged, tampered, and cross-target evidence tests live in `cli/test/auth.test.mjs` and MCP/service boundary suites. A commit is claimed only when the relevant worktree remains clean at the same commit through scan completion. |
...
| RR-EVD-03 | Fixed | Suppression IDs are recomputed and bind the full finding fingerprint; neither repository baselines nor suppressions can make findings non-actionable without trusted invocation opt-in. `cli/test/policy.test.mjs` covers tampering, baseline bypass, default denial, and approved use. |
...
| RR-APR-02 | Fixed | Approval attribution and all exact proposal bindings are covered by a host-local HMAC. The plan itself must authenticate before approval. Approval creation, CLI verification, dashboard publication, and GitHub publishing must supply their own canonical repository target; copied cross-repository bundles, tampered plans, and forged public approval IDs fail the CLI and service regressions. |
...
| RR-MCP-01 | Fixed | MCP validates bounded single-line arguments, places positional values after `--`, emits flags inline, and confines approved policy writes to the two dedicated policy filenames; option-like arguments and unrelated-file overwrite attempts are rejected before the CLI boundary. A seven-operation argv regression covers every scanning or mutating MCP-to-CLI route. |
...
| RR-MCP-03 | Fixed | A canonical-path lock rejects concurrent repository scans and `verify_fix` rescans before evidence writes can interleave within one MCP server process. |
