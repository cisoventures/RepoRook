...
| RR-APR-02 | Fixed | Approval attribution and all exact proposal bindings are covered by a host-local HMAC. Approval creation, CLI verification, dashboard publication, and GitHub publishing must supply their own canonical repository target; copied cross-repository bundles and forged public approval IDs fail the CLI and service regressions. |
...
| RR-MCP-01 | Fixed | MCP validates bounded single-line arguments, places positional values after `--`, emits flags inline, and confines approved policy writes to the two dedicated policy filenames; option-like arguments and unrelated-file overwrite attempts are rejected before the CLI boundary. |
