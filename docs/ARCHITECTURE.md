...
Successful scanners form independent resumable checkpoints under `.reporook/cache/v1`. Reuse requires a Git commit, no relevant working-tree changes, the same RepoRook and scanner versions, the complete normalized configuration, the same changed-file scope, and an unexpired record. The default 15-minute lifetime bounds staleness from advisory databases, public rules, and mutable image tags. Cache files are size-bounded, mode `0600`, atomically replaced beneath mode-`0700` directories, never followed through symbolic links, and strictly reconstructed as untrusted JSON. A malformed or stale record is a miss. Scanner errors and unavailable results are never written. Error results are retried a bounded number of times and each successful scanner is checkpointed immediately, so an interrupted multi-scanner run can resume completed work without turning missing coverage into success. Verification bypasses all cache reads.
