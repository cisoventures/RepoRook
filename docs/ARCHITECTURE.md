...
Trivy image scanning is a separate external-target adapter. It receives at most 20 validated image references from `containerImages` only after a trusted invocation supplies `--allow-external-targets`; repository configuration cannot authorize registry access. The generic Trivy username/password environment variables are removed from both version probes and scans, while host-scoped Docker credentials remain available. RepoRook never guesses an image, builds one, or broadens the target from a Dockerfile. Tags are supported, but digests provide reproducible identity. Gitleaks scans the working tree by default and switches to redacted Git-history mode only when `gitHistory` is true. Historical and image findings retain safe provenance, bypass changed-file/path filtering that only makes sense for current repository files, and do not create misleading SARIF or MCP source locations.
