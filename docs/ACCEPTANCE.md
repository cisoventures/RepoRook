...
6. authenticated scan evidence must belong to the clean current Git HEAD before remediation can proceed;
7. the service prepares one finding-bound patch and functional plus same-scanner verification plan;
8. a stale proposal digest cannot be approved, while the exact proposal produces a durable approval receipt;
9. draft-PR publishing requires a separate confirmation and a one-repository GitHub App installation token;
10. the approved patch is materialized against the exact scanned commit in the simulated remote repository, while the local working tree remains unchanged; and
11. the resulting pull request is always a draft and carries the approved functional test and `reporook verify` commands for CI and human review.
