...
RepoRook authenticates the complete report with an HMAC that also binds the canonical repository path. Consumers in the CLI, MCP server, and local service verify it before using findings as evidence. The standalone scan receipt, deterministic priorities, remediation plan, verification receipt, and final approval receipt are authenticated the same way. The proposal remains agent-authored input until the approval receipt binds its exact bytes. A JSON file that merely matches a schema, a report copied from another repository, or repository-authored content without the host-local key is rejected.

The key defaults to `$XDG_CACHE_HOME/reporook/artifact-auth-key` or `~/.cache/reporook/artifact-auth-key`; controlled automation can instead inject the same secret `REPOROOK_AUTH_KEY` into producer and consumer processes. `XDG_CACHE_HOME` must be absolute, and the cache base plus RepoRook key directory must be real directories rather than symbolic links. Key files are opened without following links and checked again through the opened descriptor before use.
...
`schemas/priorities.schema.json` is authenticated deterministic scheduling guidance derived from policy-actionable findings. It records fix-now, fix-next, and review-later items without changing severity or scanner evidence. `schemas/remediation-plan.schema.json` is also authenticated and binds one selected finding to its fingerprint, source commit, configuration, starting scope, exact-preview requirements, approval boundary, and verification command. `schemas/remediation-proposal.schema.json` defines the exact diff and tests; `schemas/approval-receipt.schema.json` records the authenticated, repository-bound named approval that binds them.

`schemas/verification.schema.json` records an authenticated deterministic scanner resolution for one finding. It preserves both scan receipts, configuration comparison, original scanner status, and any equivalent remaining finding. Functional tests are deliberately marked `not-recorded`; scanner resolution becomes a verified fix only when the focused and relevant project tests also pass.
